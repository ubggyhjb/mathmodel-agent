...
// 内容性加粗（官方展示论文通行做法）：只允许三类 = 最终答案 / 最关键模型结论 / 需要评委记住的性能指标。
// 禁止：普通 p 值/统计量加粗、方法名单独加粗、一串裸数字逐个加粗。关键数值包进结论短语整体加粗（#kw([仅到达 27 个端点])），
// 禁止裸数字逐个加粗（#kw([15.88])、#kw([12.17]) 一串 = 等于没加粗）；摘要每问 ≤2 处（5-15%），正文关键结论句 0.5-8%。
