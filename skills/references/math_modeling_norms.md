...
| [竞赛规则与 AI 使用披露](#竞赛规则与-ai-使用披露) | `1start-mathmodel` `5writing` `6verity` |
...
- **统计口径条款（写论文前逐条自查）**：①重复测量不独立——同一对象的前后两次观测不能当独立样本做 Pearson 相关/回归，必须说明是"合并样本"还是"仅基线样本"，并给出每行的 n；②假设检验必须声明单侧/双侧（scipy 的 wilcoxon 默认双侧，用了 alternative 参数就要在论文写出来）；③引用的均值/统计量必须声明口径（训练集 or 全部样本？调整前 or 调整后？）——用"调整后均值"衬托预测值属于口径误导；④样本外推必须显式声明（如用成人样本预测儿童），并给出外推风险评估；⑤相关/回归中出现的负相关与结论方向矛盾时，必须解释或改结论，不得回避；⑥比较基线必须显式声明——所有百分比、提升率、提前量、相对变化必须写明与谁相比（较 A 源单独作用/较基线模型/较调整前），换基线会得到不同数字，不写清基线的百分比不可复核。
...
- 数据读取后先检查编码、列名、形状、单位、缺失值和异常值。不要在数据错位或列名乱码时继续建模。缺失按"插值写方法+比例"处理（单字段缺失 >20% 考虑剔除并声明）；异常按"先解释后剔除且留证"处理；整列/整附件不可用必须上报用户并在论文声明降级口径（详见 2analysis 数据质量报告小节）。
...
## 图表自检清单（吸收 Bertin/Tufte 数据可视化原则 + dataviz-critique 检查口径，盲评"图表规范"维度与 style_audit 据此打分）

- 自解释性：不看正文也能从图+图注读出"谁、什么、何时、结论"；图注写明变量与统计口径（n、单位、窗口）。
- 墨水最小化（Tufte）：删掉不承载信息的元素——背景色、多余网格线、装饰边框；data-ink 比例最大化。
- 无 chartjunk：禁止 3D 立体柱、阴影、渐变、装饰插图、emoji。
- 灰度可辨：核心信息不能只靠颜色区分，线型/标记要同时编码（实线/虚线、圆/方标记）。
- 色板纪律：蓝橙双强调色 + 灰；热力图用 viridis/magma 类感知均匀色图，禁彩虹 jet。
- 字号：图内文字 ≥ 正文 0.75 倍（正文 12pt → 图内 9pt）；坐标轴标签带单位。
- 刻度规范：坐标轴从 0 开始，截断必须显式标注；图例不遮挡数据区。
- 信息密度：每张图只回答一个问题；一页内子图 ≤6 个，子图用 (a)(b) 标注。
- 图源合规：矢量优先（PDF/SVG）；位图 ≥300dpi；截图禁止；图注图下方居中、表注表上方居中。
- 图文衔接：图前有引出句（"如图 X 所示"置于图前段落）、图后有解读句；图表紧跟首次引用段落。

## 非数据图工具选择

- DrawIO 适合技术路线图、子问题求解流程图、数据处理 Pipeline、指标体系层次图、模型选择决策树、甘特图、小规模网络拓扑和简单概念框架（本机无 CLI，源文件保留、渲染走 TikZ/mermaid）。
- TikZ 适合需要精确数学标注、公式节点、复杂连线、2D 几何、变量关系、因果路径、模型架构或自定义算法流程的图。
- Mermaid（`mmdc` 已装）适合时序图、泳道图、状态机图、象限图——SVG 进 Word/Typst，PNG 进 LaTeX。
...
- 速查：需要公式或精确连线时优先 TikZ；需要时序/泳道/状态结构时优先 mermaid；需要真实场景或复杂 3D 视觉时才考虑生成式图片。
...
- 论文数字与结果记录的一致性必须由程序化追溯兜底（6verity 的 `trace_numbers.py --strict`），人工抽查不可替代；未命中结果 JSON 的论文数字都要有白名单来源说明。
...
依据《全国大学生数学建模竞赛论文格式规范（2026 年修订稿）》与《人工智能工具使用规定（2026 年试行）》：

- **摘要页**：标题+摘要+关键词同页（摘要原则上不超过一页，无需翻译成英文）；页码从此页起"1"、页脚中部。
- **正文**：不要目录；不超过 30 页；附录页数不限。
- **附录**：必须含**支撑材料文件列表** + 建模所用全部完整可运行源码；缺程序/程序不能运行/运行结果与论文不符 → 可能取消评奖资格。
- **电子版**：单一 PDF 或 Word（建议 PDF）≤20MB、不压缩；**第一页必须为摘要专用页**（承诺书/编号页不放电子版）。
- **支撑材料**：RAR/ZIP ≤20MB，含可运行源码、自主查阅的数据资料、较大篇幅中间结果图表；文件列表放论文附录；与论文不符 → 可能取消资格。
- **匿名**：摘要/正文/附录任何地方不得出现身份、学校、赛区信息。
- **AI 使用声明（2026 试行，2026-09-01 起）**：论文**参考文献前**必须设"AI 工具使用声明"，用官方定句二选一——未使用："本参赛队在竞赛过程中未使用任何 AI 工具。"；使用："本参赛队在竞赛过程中使用了 AI 工具，主要用于【简要用途，如语言润色、代码调试等】，详细使用情况见支撑材料。"。使用 AI 的须在**支撑材料**中放入详情文件，**文件名必须为"AI 工具使用详情.pdf"**，内容 4 项：① 工具名称、版本或型号；② 具体使用目的和环节；③ 主要提示方式与使用过程说明（可附典型交互示例）；④ 对 AI 输出的采纳、人工修改和核验的主要情况（**语言润色除外**）。故意隐瞒、虚假声明，或将未经核验的 AI 生成内容直接作为核心建模与分析成果提交 → 取消评奖资格。
- **引用**：所有引用他人或公开资料（含网上资料）必须按科技论文规范列参考文献并在正文标注，不得大篇幅照抄。

### 《参赛规则》（2026 修订稿）与《章程》（2023 修订稿）要点

- **评奖四标准（章程第 2 条，官方唯一评奖口径）**：假设的合理性、建模的创造性、结果的正确性、文字表述的清晰程度。本模式盲评 8 维表与此四标准映射：假设→合理性、建模求解→创造性、结果检验→正确性、结构表述+图表→清晰性。
- **组队（章程第 3 条 + 2026 第一次通知）**：每队不超过 3 人且须同一所学校；分本科组/高职高专组（报名后组别不可改）；**研究生不得参加**；2026 年竞赛时间 9 月 10 日（四）18:00 – 9 月 13 日（日）20:00，报名截止 9 月 7 日 20:00（管理系统 cumcm.cnki.net）；参赛费每队 50 元（全国层面）。
- **独立完成（参赛规则第 5 条，最严纪律）**：竞赛期间不得以任何方式与队外任何人（包括指导教师）交流、讨论与赛题有关的问题（主动参与与被动接收讨论信息均视为违纪）；**不得在交流平台（贴吧、QQ 群、微信群、网络直播间、知乎、小红书、CSDN、GitHub 等）浏览、发布或讨论与赛题相关的内容——浏览即违纪**。允许使用各种图书资料（包括互联网上的公开资料）、计算机和软件（章程第 3.3 条）——学术文献与官方数据检索合法，交流平台赛题讨论非法。
- **指导教师（参赛规则第 3 条）**：竞赛期间不得通过任何方式对学生进行任何形式指导（解释赛题、选题/解题建议、提供资料、修改论文或建议等），否则按违纪处理，两年内不受理该指导教师报名。
- **抄袭（参赛规则第 4 条）**：引用他人研究成果或公开资料必须按规范在正文标注并列入参考文献，且不得大篇幅照抄，否则取消评奖资格。
- **AI（参赛规则第 6 条）**：允许使用 AI 工具辅助，参赛队对作品原创性、真实性和准确性负全部责任，须遵守 AI 使用规定。

### 唐宏岩（组委会）AI 讲评要点（校准 AI 纪律）

- 学生自报 AI 使用率 **97.8%**；**37.6% 学生无法判断 AI 输出对错**（评审危机根源）。AI 幻觉 = 编造文献/结果、偷换数据、忽略关键约束、虚假因果。
- AI 强项在执行类环节（摘要/润色/翻译/排版、编程、检索综述），弱项在认知类环节（核心建模、问题抽象、逻辑一致性、多步推导、独立迭代）。
- 组委会实验：让 AI 直接完成 2022E/2023D 真题 → "总体质量尚可，但**远未达到获国奖水平**"（表面结构完整、图文并茂，实则内容空泛、逻辑断层、对约束视而不见）。
- 顶尖队伍对 AI 评价最高在**代码/数据处理/建模**环节、**最低在排版环节**——AI 用于执行类补强，而非替代建模思考。
- 评审趋势（按讲评）：① 摘要必须含**可验证的定量指标**；② 关键结论必须有**可引用出处或推导过程**，消除无依据断言；③ 提交建模过程与 **AI 交互记录**附件；④ **逻辑一致性审查**（假设-模型-数据-算法-结论自洽）；⑤ 高奖候选复核：抽查**代码/数据/结果一致性** + 答辩质询。

### 数据真实性与引用

- 严禁虚构、篡改数据或结果。论文所有数字必须能追溯到题面附件或自证通过的代码输出（`verify_all.py` + `trace_numbers.py` 兜底）；"编造一个看起来合理的数字"与"算错一个数字"在评审面前同罪。
- 参考文献必须真实存在且可核实，禁止编造引用；禁止大段抄袭他人论文、代码或图表。
- 竞赛期间不得在公开平台（公开仓库、论坛、博客、社交平台）讨论题目思路、数据或解法。工作目录与产物在竞赛结束前不要公开。
- **匿名要求**：论文内不得出现学校、姓名、指导教师等身份信息。
